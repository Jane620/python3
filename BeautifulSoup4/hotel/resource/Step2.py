...
def anaysis_info(content):
    ...
    for j in range(len(a)):
        ...
        hotel_addr = a[j]('p',attrs={'class':'hotel_item_htladdress'})
        if hotel_addr:
            hotel_addr = ''.join(hotel_addr[0].text.split(' ')[:-1])
        ...
        try:
            ...
            continue
        hotel_detail[hotel_name] = [hotel_stars,hotel_addr,hotel_style]
    return hotel_detail
...
def create_file(province,file_path):
    ...
    row0 = [u'省份', u'市', u'酒店名', u'星级', u'地址',u'类型']
